...
def manual_coalesce(edge_index, edge_attr, size=None):
    # Convert to CPU numpy for ease
    src, dst = edge_index.numpy()
    attrs = edge_attr.numpy()

    # Create a DataFrame to group by (src, dst)
    df = pd.DataFrame({
        'src': src,
        'dst': dst,
        'attr': list(attrs)
    ...
    # Group by (src, dst) and sum attributes
    grouped = df.groupby(['src', 'dst'], sort=False)['attr'].apply(
        lambda x: torch.stack(list(x)).sum(dim=0)
    ).reset_index()

    # Convert back to torch tensors
    new_edge_index = torch.tensor([grouped['src'].tolist(), grouped['dst'].tolist()], dtype=torch.long)
    new_edge_attr = torch.stack(grouped['attr'].tolist())
...
def mol_to_graph_backend(atmfeat, coord, bond, bondfeat, props={}, data_cls=Data):
    ...
    if bond.shape[0] > 0:
        ...
        edge_index, edge_attr = manual_coalesce(edge_index, edge_attr)

...
def process_all_folders_res(all_folders_res):
    ...
    for  class_folder, class_folder_value in all_folders_res.items():
        if 'pret' not in class_folder:
            continue
        if not class_folder_value:
            continue 
        all_res[class_folder] = {}
        for inference_file_name, inference_file_name_value in all_folders_res[class_folder].items():
            result_in_one_file = []
            for column_name, column_name_value in all_folders_res[class_folder][inference_file_name].items():
                this_pred_label = all_folders_res[class_folder][inference_file_name][column_name]['predictions']
            ...
            all_res[class_folder][inference_file_name] = np.concatenate(result_in_one_file, axis=1)
    return all_res
...
Parser.add_argument('--save_path', default='/home/msrai4srl4s/yuwang5/Evaluation/bio_t5/new_pipeline/res', type=str)
...
Parser.add_argument('--fail_strategy', default='mask', type=str)    

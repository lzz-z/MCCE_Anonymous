...
def smiles_to_selfies(smiles):
    try:
        res =  sf.encoder(smiles, strict=False)
    except:
        res = None
    return res
...
def get_all_prediction(input_smiles_all, task, tokenizer, model, batch_size=2560, args=None):
    ...
    pred_sfi = [smiles_to_selfies(vs) for vs in pred_smi_can_all]
    mask = [True if x is not None else False for x in pred_sfi]

    valid_sfi = [pred_sfi[i] for i in range(len(input_smiles_all)) if mask[i]]
    valid_sfi_v = [x for x in pred_sfi if x is not None ]
    assert valid_sfi_v == valid_sfi
    ...
    # print(valid_input_smiles_can)

    result = []
    input_smi_can_index = 0
    for i in range(len(input_smiles_all)):
        if mask[i]:
            result.append(pred_smi_can_all[input_smi_can_index])
            input_smi_can_index += 1
        else:
            result.append(None)
    all_res['input_smi_can'] = result
    ...
    n_valid_mol = len(valid_sfi)
    if n_valid_mol>0:
        t5_input = [ get_t5_input(s, task) for s in valid_sfi]
        ...
        split_idx = split_list_by_length_indices(list(range(n_valid_mol)), batch_size)
        all_prediction = []
    ...
    assert len(result) == len(input_smiles_all)
    return all_res
...
def evaluate_one_task(task_file, task, hparams):
    ...
    this_res = evaluate_one_file(task_file, task, tokenizer, model, batch_size, hparams)
...
if __name__ == '__main__':
    ...
    Parser.add_argument('--ckpt_blob_root', default='/home/msrai4srl4s/yuwang5/blob/blob/', help='blob root for bioT5 model checkpoint')
    Parser.add_argument('--inference_blob_root', default='/home/msrai4srl4s/yuwang5/blob/rl4s/', help='blob root for inference results')
    Parser.add_argument('--inference_file', default='/home/msrai4srl4s/yuwang5/Evaluation/data/yince_20240905_bbbp.txt', help='inference folder path should include the folder with task name')
    ...
    one_task_res = evaluate_one_task(f'{hparams.inference_file}', hparams.task, hparams)
    

    with open(f'{hparams.inference_file}_res.csv', 'wb') as f:
        pd.DataFrame({'input_mol' : one_task_res['input_mol']['input_smiles'], 'predictions': one_task_res['input_mol']['predictions']}).to_csv(f)
        print(f'save to {hparams.inference_file}_res.csv')
    # # 检查文件夹是否存在，不存在提前创建
    # if not os.path.exists(f'{hparams.save_path}'):
    #     os.makedirs(f'{hparams.save_path}')
    # print(f'save to {hparams.save_path}/task_{hparams.task}_res.pkl')
    # with open(f'{hparams.save_path}/task_{hparams.task}_res.pkl', 'wb') as f:

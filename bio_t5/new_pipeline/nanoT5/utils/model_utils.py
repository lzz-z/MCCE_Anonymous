...
class T5_encoder_with_head(nn.Module):
    def __init__(self, args, t5_model: T5ForConditionalGeneration, config):
        super().__init__()
        self.args = args
        self.model = t5_model.encoder
        self.classification_head = T5ClassificationHead(config)
        # if args.data.data_dir.split('/')[-1] == 'esol':
        #     ckpt = torch.load('/protein/users/v-qizhipei/checkpoints/unimpt/finetune_molnet_reg_only_encoder_with_head/esol/0904_regl2_gpu1_bsz16_acc2_ts50000_eps100_ws1000_cosine_lr1e-3_dp0.1_seed12/checkpoint-ft-2000/pytorch_model.bin', map_location='cpu')
        #     ckpt = {k: v for k, v in ckpt.items() if 'classification_head' in k}
        #     ckpt = {k.replace('classification_head.', ''): v for k, v in ckpt.items()}
        #     self.classification_head.load_state_dict(ckpt, strict=True)
        # elif args.data.data_dir.split('/')[-1] == 'freesolv':
        #     ckpt = torch.load('/protein/users/v-qizhipei/checkpoints/unimpt/finetune_molnet_reg_only_encoder_with_head/freesolv/0904_regl1_gpu1_bsz8_acc1_ts50000_eps100_ws1000_cosine_lr1e-4_dp0.1_seed12/checkpoint-ft-2000/pytorch_model.bin', map_location='cpu')
        #     ckpt = {k: v for k, v in ckpt.items() if 'classification_head' in k}
        #     ckpt = {k.replace('classification_head.', ''): v for k, v in ckpt.items()}
        #     self.classification_head.load_state_dict(ckpt, strict=True)
        # elif args.data.data_dir.split('/')[-1] == 'lipo':
        #     ckpt = torch.load('/protein/users/v-qizhipei/checkpoints/unimpt/finetune_molnet_reg_only_encoder_with_head/lipo/0904_regl2_gpu1_bsz16_acc2_ts50000_eps100_ws1000_cosine_lr3e-4_dp0.1_seed2/checkpoint-ft-12000/pytorch_model.bin', map_location='cpu')
        #     ckpt = {k: v for k, v in ckpt.items() if 'classification_head' in k}
        #     ckpt = {k.replace('classification_head.', ''): v for k, v in ckpt.items()}
        #     self.classification_head.load_state_dict(ckpt, strict=True) 
        if args.reg_loss == 'l1':
    ...
    def forward(self, input_ids, attention_mask, labels):
        ...
        # use <bom> as the representation of the whole sequence
        encoder_last_hidden_states = outputs['last_hidden_state'][:, 0, :]
        logits = self.classification_head(encoder_last_hidden_states)
...
def get_model(args, config, tokenizer, logger):
    ...
    if args.model.only_encoder_with_head:
        # For regression
        config.num_labels = 1
        model = T5_encoder_with_head(args, model, config)
    return model
...
def get_tokenizer(args):
    ...
    selfies_dict_list = [line.strip() for line in open(os.path.join(__file__.split('nanoT5/utils')[0], args.molecule_dict))]
...
def load_dataset_splits(args):
    if args.mode == 'pt':
        ...
        print(os.path.exists(args.data.data_dir))
        print(len(dataset_splits['test']) , args.data.data_dir,args.data.task_dir,'@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@')
        # assert 1==2
...
def get_data_collator(tokenizer, config, args):
    if args.mode == 'pt':
        ...
        if args.model.only_encoder_with_head:
            data_collator = DataCollatorForNIOnlyEncoderWithHead(
                tokenizer,
                padding="longest",
...
def get_dataloaders(tokenizer, config, args):
    if args.mode == 'pt':
        ...
        # print(dataset_splits['train'][0],'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!', args)
        # assert 1==2

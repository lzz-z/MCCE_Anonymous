...
def extract_number_re(s):
    ...
    match = re.search(r'sft_epoch_(\d+)_dpo_epoch_(\d+)', s)
    if match:
        sft_epoch_num = int(match.group(1))  # The number right after 'epoch'
        dpo_epoch_num = int(match.group(2))  # The number after the underscore
        return (sft_epoch_num, dpo_epoch_num)
...
# for task in ['bace', 'bbbp']:
# for task_folder in all_task_folders:
for task_folder in ['bace', 'bbbp']:
    res = {}
    test_data_path = f'/home/yuwang5/blob/rl4s/Data/202407/{task_folder}/dpo_{task_folder}_test.csv'
...
#     continue
    ...
    res[task_folder] = {}
    for task_class_folder in sorted(os.listdir(os.path.join(root_path, task_folder)), key=extract_number_re):
        ...
        for inference_file_name in sorted(os.listdir(os.path.join(root_path, task_folder, task_class_folder)), key=extract_number_re):
            all_folders[task_folder][task_class_folder][inference_file_name] =  sorted(os.listdir(os.path.join(root_path, task_folder, task_class_folder, inference_file_name)), key=extract_number_re)
            all_result = []
            for file_name in all_folders[task_folder][task_class_folder][inference_file_name]:
                try:
                    data_path = os.path.join(root_path, task_folder, task_class_folder, inference_file_name, file_name, 'biot5_pred_yes_no.csv')
                    ...
                    this_res = np.zeros(len(true_label))
    ...
    all_folders_res[task_folder] = res
                


# 遍历整个res, 计算每个task_folder下的task_class_folder的inference_file_name的success rate
def get_evaluation_results(res, task_name, type='mean', column='3:', sft_epoch_for_dpo=[1,2,3]):
    all_res = {}
    # for task_folder in all_folders:
    for task_class_folder in all_folders[task_name]:
        for inference_file_name in all_folders[task_name][task_class_folder]:
            this_res = eval(f'res[task_name][task_class_folder][inference_file_name][{column}]')
            ...
            all_res[f'{task_name} {task_class_folder} {inference_file_name} success rate:'] =  success_rate  
                # assert 1==2
